...
#compute layer thickness for GHB calculation
lay_thick = [top - lay1, lay1 - lay2, lay2 - lay3, lay3 - lay4, lay4 - lay5, lay5 - lay6, lay6 - lay7, lay7 - lay8, lay8 - lay9]
...
for k in range(49,50):
    for l in range(0,7):
        ghb_spd += [[l, i, k, 0, k1*lay_thick[l]*(delr/500)] for i in range(nrow)]
...
def plot_mf():
    ...
    for idx, ax in enumerate(axes.flatten()[:nlay]):
        ...
        fmp.plot_bc("RIV", color="blue", plotAll=True)
        fmp.plot_bc("GHB", color="orange", plotAll=True)
