...
lay_disc = 1  # layer discretization and model thickness in meters 
              ## for above holstein*

perlen = 1*365  # Zeit für Flow-Modell
perlent = 10*365 # Zeit für Transportmodell
...
frate_fl = -720 # Förderrate flache Brunnen
frate_tf = -600 # Förderrate tiefer Brunnen
...
ws = './October_pump_'+str(frate_fl*-4+frate_tf*-1)+'_Q'
...
if lay_disc == 2:
    ...
    ibound[23:24, 0:50, 0] = -1
    ibound[23:24, 0:50, 49] = -1
    # Extract concentrations of all wells from .UCN files and create model cross section with concentration overlay
    fname = os.path.join(orig, "MT3D001.UCN")
    ucnobj = flopy.utils.UcnFile(fname)
    times = ucnobj.get_times()
    # conc = ucnobj.get_alldata()
    orig_cnc = ucnobj.get_data(totim=times[-1]) # starting concentrations 
    hds_orig = bf.HeadFile(os.path.join(orig, modelname_mf + '.hds'))
    times = hds_orig.get_times()
    strt = hds_orig.get_data(totim=times[-1])
    # strt = np.ones((nlay, nrow, ncol), dtype=np.float32)
    # strt[23:24, 0:49, 0] = strt_head_u
    # strt[23:24, 0:49, 49] = strt_head_u  # Starting head ($m$)
...
plt.ylabel(("Chlorid (kg/day)"), fontsize=15)
plt.rcParams.update({'font.size': 15})
...
##plt.ylim(0, 800)
plt.xlabel("Zeit (jahr)", fontsize=15)
plt.ylabel(("Chlorid load (kg/day)"), fontsize=15)
plt.rcParams.update({'font.size': 15})
plt.grid(visible=True)
# title1 = "Chlorid-Konzentrationen\n in der GHBvariante des KHM Friedrichshagen"
# title2 = "Filterlänge: Brunnen A, B, D, E = {} m, Brunnen C = {} m und " \
